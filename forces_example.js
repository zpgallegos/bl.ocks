function createBubbleChart(error, countries, continentNames) {
  var populations = countries.map(function (country) {
  ...
  var circleRadiusScale = d3.scaleSqrt().domain(populationExtent).range([circleSize.min, circleSize.max])
  ...
  function createSVG() {
    svg = d3.select("#bubble-chart").append("svg").attr("width", width).attr("height", height)
  }
  ...
  function addGroupingListeners() {
    ...
    function togglePopulationAxes(showAxes) {
      ...
      function createAxes() {
        ...
        var xAxis = d3.axisBottom(populationScaleX).ticks(numberOfTicks, tickFormat)
        ...
        var yAxis = d3.axisLeft(populationScaleY).ticks(numberOfTicks, tickFormat)
        svg
          .append("g")
          .attr("class", "y-axis")
